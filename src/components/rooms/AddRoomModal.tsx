...
import type { ApiRoomType } from '../../types/admin';
...
export default function AddRoomModal({ roomTypes, onClose, onCreated }: Props) {
  const [roomNumber, setRoomNumber] = useState('');
  const [roomTypeId, setRoomTypeId] = useState<string>(roomTypes.length ? String(roomTypes[0].id) : '');
    ...
    try {
      ...
      setError(
        err instanceof ApiError && err.status === 400
          ...
          : err instanceof ApiError ? err.message : 'No se pudo crear la Habitación. Intenta de nuevo.',
      );
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-surface rounded-2xl shadow-xl border border-outline-variant/20 overflow-hidden">
        <div className="flex items-center justify-between px-8 py-6 border-b border-outline-variant/15">
          <div>
            <h2 className="text-lg font-extrabold text-primary tracking-tight">Agregar Habitación</h2>
            <p className="text-xs text-on-surface-variant mt-0.5">
              Registra una nueva habitación física.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="px-8 py-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" placeholder="Ej. 101" value={roomNumber} onChange={e => setRoomNumber(e.target.value)} disabled={isSubmitting} className={inputCls} required />
            </div>
            <div>
              <label className={labelCls}>Piso</label>
              <input type="number" min="0" placeholder="1" value={floor} onChange={e => setFloor(e.target.value ? Number(e.target.value) : '')} disabled={isSubmitting} className={inputCls} required />
            </div>
          </div>
          ...
          <div>
            <label className="flex items-center gap-3 select-none cursor-pointer">
              ...
              <span className="text-sm font-bold text-on-surface">Marcar como Activa</span>
            </label>
          </div>

          <div>
            <label className={labelCls}>Notas (Opcional)</label>
            <textarea placeholder="Ej. Vista al mar..." value={notes} onChange={e => setNotes(e.target.value)} disabled={isSubmitting} className={inputCls} rows={2} />
          </div>
